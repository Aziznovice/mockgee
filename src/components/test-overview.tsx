...
import { Clock, ListChecks, Pencil, Play } from "lucide-react";
...
export function TestOverview({ test, onStart }: TestOverviewProps) {
    ...
    const initialMinutes = test.duration ? test.duration % 60 : 30; // Default to 30 mins if no duration

    const [useTimer, setUseTimer] = useState(!!test.duration);
    ...
    const handleStart = () => {
        onStart(useTimer && totalDurationInMinutes > 0 ? totalDurationInMinutes : null);
    }
                    ...
                    <div className="space-y-4 p-4 rounded-lg border bg-muted/50">
                     ...
                     <div className="flex flex-col items-center gap-4 rounded-lg border bg-muted/50 p-4">
                        <div className="flex items-center justify-between w-full">
                            <Label htmlFor="timer-switch" className="text-lg flex items-center gap-2"><Clock className="h-5 w-5"/>Time Limit</Label>
                            <Switch id="timer-switch" checked={useTimer} onCheckedChange={setUseTimer} />
                        </div>
                        {useTimer && (
                             <div className="flex items-center gap-2 pt-2">
                                <div className="text-2xl font-bold p-2 rounded-md bg-background min-w-[120px] text-center">
                                    {totalDurationInMinutes > 0 ? `${totalDurationInMinutes} min` : 'No limit'}
                                </div>
